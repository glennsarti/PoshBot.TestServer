# Originally from https://github.com/proxb/PoshChat
...
Function Invoke-PostBotTestServer {
  [cmdletbinding()]
  Param (
    [parameter()]
...
#Write-Host "Remove!!"
        ...
        # Make sure connections are alive
        $deleteUserList = $Clienthash.GetEnumerator() | ForEach {
          $Username = $_.Name
          if (-not $Clienthash[$Username].Connected) { Write-Output $Username }
        }
        $deleteUserList | ForEach {
          Write-Host "$_ is no longer connected"
          $shareddata.Remove($_)
          $clienthash.Remove($_)
          $RemoveQueue.Enqueue($_)
        }

      } catch {
        $ErrorMessage = $_.Exception.Message
        Write-Warning "Error in RemoveQueue: ${ErrorMessage}"
      }
    }
          ...
          If (-Not ($ClientHash.Contains($_.Name))) {
            ...
            $sb = {
              ...
              While ($True) {
                ...
                # The Read buffer may be corrupt because the connection is no longer live
                If (($return -gt 0) -and ($Client.Connected)) {
          ...
          If ($directMessage) {
            ...
            $Clienthash.GetEnumerator() | ForEach {
              ...
              Try {
                ...
                $ErrorMessage = $_.Exception.Message
                Write-Warning "Error sending message to '${Username}': ${ErrorMessage}.  Removing the client"
                $RemoveQueue.Enqueue($Username)
              }
            }
          }
        }
    ...
    # Spin wait for the server to die
    Do {
